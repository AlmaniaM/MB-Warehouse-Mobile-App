...
export class LedgerSummaryPage implements OnInit {
  ...
  filterLedger(force: boolean = false) {
    ...
    // Use the date range filter method
  ...
  resetFilters() {
    ...
    this.loadLedgerEntries(true); // Force refresh when resetting filters
  }
  // Method to refresh all data (force reload)
  refreshAllData() {
    this.loadContainerTypes(true);
    this.loadLedgerEntries(true);
  }

  // Handle new ledger entry creation
